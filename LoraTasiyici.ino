...
typedef  struct 
{
  ...
  float tas_pressure = 0.00;
  float tas_altitude = 0.00;
} Signal;
...
void loop()
{
  ...
  Serial.println();
    ...
    Serial.print(bmp.readAltitude(900.30)); /* Adjusted to local forecast! */
    Serial.println(" m");
    data.tas_altitude = bmp.readAltitude(900.30);

    Serial.println();

  portlora.listen(); 

  ResponseStatus rs = e32ttl.sendFixedMessage(0, 57, 28, &data, sizeof(Signal));
